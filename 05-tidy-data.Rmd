...
```{r}
# oops, lubridate has here too...
index <- read_csv(file = here::here("data/consumer_index.csv"))
index
```
...
Real-life example:
```{r}
(rba <- read_csv(here::here("data/ECIS_141022_MFT_2_RbA.csv")))
```
...
```{r}
(headers <- read.csv(here::here("data/ECIS_141022_MFT_2_RbA.csv"), 
                     skip = 19, 
                     header = FALSE, 
                     nrows = 2, 
...
```{r}
(sheets <- excel_sheets(here::here("data/ECIS_141022_MFT_1.xlsx")))
```

You need to import one sheet at the time:
```{r}
(z_500 <- read_excel(here::here("data/ECIS_141022_MFT_1.xlsx"), 
                     sheet = "Z 500 Hz"))
```

```{r, eval = FALSE}
(z <- read_excel(here::here("data/ECIS_141022_MFT_1.xlsx"), 
                 sheet = 3:5))
```
...
```{r}
mft <- map(sheets[3:5], ~ read_excel(here::here("data/ECIS_141022_MFT_1.xlsx"), sheet = .x))
...
```{r}
(salary <- read_excel(here::here("data/keskmine_brutokuupalk.xls"), 
                    skip = 4))
...
```{r}
trans <- read_csv(here::here("data/transactions.csv"))
trans
```
