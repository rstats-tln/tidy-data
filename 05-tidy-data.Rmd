...
A common problem is a dataset where some of the column names are not names of variables, but values of a variable (wide data). 
...
salary
salary <- rename(salary, month_est = ...1)
salary
```
...
Fir this we can use `pivot_longer()` function:
```{r}
salary_tidy <- salary %>% 
...
Let's merge salay data with average price per m^2 from transactions data:
```{r}
...
To combine the tidied versions of salary and transactions into a single tibble, we need to use dplyr::left_join()
```{r}
...
sp <- salary_and_prices %>% 
  ...
  pivot_longer(cols = c("value", "av_price"))
sp
```
...
```{r}
pivot_wider(sp, names_from = name, values_from = value)
```

Missing observations are filled in with NA-s:
```{r}
...
```{r}
Rb_tidy <- pivot_longer(Rb, cols = contains("Rb"))
Rb_tidy
```
...
Rb_separated <- Rb_tidy %>% 
   separate(name, into = c("well", "parameter"), sep = "_")
Rb_separated
```
